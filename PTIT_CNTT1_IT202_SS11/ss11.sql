...
begin
    ...
    else set p_bonus_points = p_bonus_points;
    end if;
end $$
delimiter ;
...
begin
    if char_length(p_content) < 5 then set result_message = 'noi dung qua ngan';
    else
...
begin
    ...
    select count(*) into like_count from likes l
    join posts p on l.post_id = p.post_id
    ...
    case
        when activity_score > 500 then set activity_level = 'rat tich cuc';
        when activity_score between 200 and 500 then set activity_level = 'tich cuc';
        else set activity_level = 'binh thuong';
    end case;
end $$
delimiter ;
